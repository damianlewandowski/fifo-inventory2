...
```
heroku create fifo-inventory-prod --remote prod && \
    heroku addons:create newrelic:wayne --app fifo_inventory-prod && \
    heroku addons:create heroku-postgresql:hobby-dev --app fifo_inventory-prod && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="fifo_inventory-prod" \
        --app fifo_inventory-prod
```

Creating the qa sever:

```
heroku create `fifo-inventory-qa --remote qa && \
    heroku addons:create newrelic:wayne && \
    heroku addons:create heroku-postgresql:hobby-dev && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="fifo_inventory-qa" \
```
